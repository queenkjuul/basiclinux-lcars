...
#ifdef PTR_IS_64
#define LONG_FIVES	0x5555555555555555L
#define OBJECTGRAIN     16L
#define HANDLEGRAIN     16L
#else
#define LONG_FIVES	0x55555555L
#define OBJECTGRAIN     8L
#define HANDLEGRAIN     8L
#endif
...
#define MarkPtr(p, v) _MarkPtr(((ptr_int) (p) & ~(OBJECTGRAIN - 1)), v)
#define ClearMarkPtr(p, v) _ClearMarkPtr(((ptr_int)(p)&~(OBJECTGRAIN-1)),v)
#define IsMarked(p) _IsMarked((ptr_int) (p) & ~(OBJECTGRAIN - 1))
...
#define ValidObject(p)	((((ptr_int)(p)) & (OBJECTGRAIN-1)) == 0 && 	\
			 (unsigned char *)(p) >= opmin &&	      	\
			 (unsigned char *)(p) <  opmax)
#define ValidHandle(p)	(((ptr_int) (p) & (sizeof(JHandle)-1)) == 0 && 	\
			 (unsigned char *)(p) >= hpmin &&	        \
			 (unsigned char *)(p) <= hpmax)
/* ValidHorO() assumes OBJECTGRAIN=sizeof(JHandle)... */
#define ValidHorO(p)	(((ptr_int) (p) & (OBJECTGRAIN-1)) == 0 &&	\
			 (unsigned char *)(p) >= hpmin &&		\
			 (unsigned char *)(p) <= opmax)
#define SetLimits()							\
...
#define BITOFFSET(p)	((((unsigned char *)(p) - hpmin) >> 3) & 0x1e)
#else
...
#define GetPageMapEntry(p)					  	\
	     (page_map[((ptr_int)(p) - (ptr_int)mem_base) >> PTR_2_PAGE_SHIFT])

#define ValidObject(p)	((((ptr_int)(p)) & (OBJECTGRAIN-1)) == 0 &&	\
	     (void *)(p) >= mem_base &&		    			\
	     (void *)(p) <  mem_top &&			    		\
	     (GetPageMapEntry((p)).chunk_size > 0))
#define ValidHandle(p)	(((((ptr_int)(p)) & (HANDLEGRAIN-1)) == 0) &&  	\
	     ((void *)(p) >= mem_base) &&		    		\
	     ((void *)(p) <  mem_top) &&		    		\
	     (GetPageMapEntry((p)).chunk_size < 0))
/* ValidHorO() assumes OBJECTGRAIN == HANDLEGRAIN... */
#define ValidHorO(p)	((((ptr_int)(p)) & (HANDLEGRAIN-1)) == 0 &&	\
	     (void *)(p) >= mem_base &&		    			\
	     (void *)(p) <  mem_top &&			    		\
	     (GetPageMapEntry((p)).chunk_size != 0))
...
#define MARKINDEX(p)	(((ptr_int)(p) & (PAGE_ALIGNMENT - 1)) >> 7)
#define BITOFFSET(p)	((((ptr_int)(p) & (PAGE_ALIGNMENT - 1)) >> 2) & 0x1e)
