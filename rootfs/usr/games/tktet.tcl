...
## Started: Sat Aug 11 15:27:32 PDT 1995
##
## jeff@hobbs.org
##
...
if {$tetris(WWW)} { 
    ...
    proc About {} {
	variable tetris
	tk_messageBox -icon info -title "About Tetris v$tetris(version)" \
		-type ok -message $tetris(info) -parent $tetris(root)
    }
}
...
proc Init {{base {}}} {
    ...
    # Do this next, to get the parsed values in
    array set games [list \
	    standard	[list 0 $tetris(showNext) $tetris(shadow) \
	    0 0 $tetris(growMax) 0] \
	    speed	[list 15 1 0 0  0 $tetris(growMax) 0] \
	    row		[list 10 1 1 1 10 10 20] \
	    rowfast	[list 10 1 1 1 10 20 11] \
	    wizard	[list 10 0 0 1 15 20 15] \
	    ]
    ...
    bind $widget(board) <1> [list focus $widget(board)]
...
proc GameTypes {} {
	...
	radiobutton $w.b0 -text "Standard Tetris" -anchor w \
		-variable $tetris(name)tetris(game) -value "standard" \
		-command [namespace code SetGame]
	radiobutton $w.b1 -text "I'm on Speeeeed" -anchor w \
		-variable $tetris(name)tetris(game) -value "speed" \
		-command [namespace code SetGame]
	radiobutton $w.b2 -text "Row, Row, Row your boat" -anchor w \
		-variable $tetris(name)tetris(game) -value "row" \
		-command [namespace code SetGame]
	radiobutton $w.b3 -text "Row faster ..." -anchor w \
		-variable $tetris(name)tetris(game) -value "rowfast" \
		-command [namespace code SetGame]
	radiobutton $w.b4 -text "I'm the Pinball Wizard" -anchor w \
		-variable $tetris(name)tetris(game) -value "wizard" \
		-command [namespace code SetGame]
...
proc SetGame {{type {}}} {
	...
	set type $tetris(game)
    }
    if {![info exists games($type)]} {
	set type "standard"
    }
    set tetris(game) $type
    foreach {
	tetris(initLevel) tetris(showNext) tetris(shadow)
	tetris(growing) tetris(growMax) tetris(growLevel) tetris(startRows)
    } $games($type) {
	break
    }
    Reset
