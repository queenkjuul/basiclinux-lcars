...
(defun mkEntry2 (&optional (w '.e2)) 
    ...
    (entry (conc w '.frame.e1) :relief "sunken" :scroll (tk-conc w ".frame.s1 set"))
    (scrollbar (conc w '.frame.s1) :relief "sunken" :orient "horiz" :command 
	    (tk-conc w ".frame.e1 view"))
    (frame (conc w '.frame.f1) :geometry "20x10")
    (entry (conc w '.frame.e2) :relief "sunken" :scroll (tk-conc w ".frame.s2 set"))
    (scrollbar (conc w '.frame.s2) :relief "sunken" :orient "horiz" :command 
	    (tk-conc w ".frame.e2 view"))
    (frame (conc w '.frame.f2) :geometry "20x10")
    (entry (conc w '.frame.e3) :relief "sunken" :scroll (tk-conc w ".frame.s3 set"))
    (scrollbar (conc w '.frame.s3) :relief "sunken" :orient "horiz" :command 
	    (tk-conc w ".frame.e3 view"))
